# -----------------------------------------------------
# 喵喵实战室：Agent组装车间 (main_with_memory.py)
# -----------------------------------------------------
...
# -----------------------------------------------------
# 步骤3：将Python函数“封装”成Agent能用的Tool(与上一期相同) 
# -----------------------------------------------------
print("[2/5] 正在封装工具 '双手'...")
...
# 将所有工具的名称提取出来，方便后面提示词使用
tool_names = ", ".join([tool.name for tool in tools])

# -----------------------------------------------------
# 步骤4：创建 ReAct 提示词模板 (Agent的“行动纲领”)
# -----------------------------------------------------
print("[3/5] 正在加载 ReAct 思考框架(加入记忆)...")

# 这段提示词是LangChain提供的标准ReAct模板
# 它告诉Agent如何思考、如何使用工具、如何给出最终答案
# 我们从Hub上拉取一个中文优化过的模板（如果需要，或使用默认英文模板）
# ！！！注意看，它比上一期的模板多了 {chat_history} 变量！！！
# 简化的模板示意：
...
# -----------------------------------------------------
# 步骤5：创建Agent执行器 (与上一期相同)
# -----------------------------------------------------
print("[4/5] 正在组装Agent执行器...")
...
# 步骤6：！！！创建“记忆芯片”和“对话循环”！！！
# -----------------------------------------------------
print("[5/5] 正在创建 '短期记忆芯片'...")

# 在这里，我们初始化一个空的聊天记录对象
# ChatMessageHistory 是一个简单的内存中存储
chat_history = ChatMessageHistory()

print("\n--- 具备记忆的Agent 已启动，请输入你的问题 ---")
print("--- (输入 '退出' 来结束对话) ---")
...
while True:
    try:
        ...
        # 2. ！！！调用Agent执行器！！！
        # 关键：我们把“历史记录”和“新输入”一起传进去
        result = agent_executor.invoke({
            "input": user_input,
            "chat_history": chat_history.messages # 传入完整的历史消息列表
        })
